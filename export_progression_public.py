...
- Filtre: n'affiche que Date <= aujourd'hui
...
# ========= DEBUG =========
VERSION = "export_progression_public.py :: 2025-10-29 (docs/)"
DEBUG = True
def dbg(msg: str): 
    if DEBUG: 
        print("[DEBUG]", msg)
...
CLASSES = {
    "407": {
        "level_subdir": "College",  # respecte la casse utilisée en ligne
...
def harmonize_headers(df: pd.DataFrame, code: str) -> pd.DataFrame:
    ...
    if missing: dbg(f"ATTENTION({code}) colonnes manquantes: {missing} | présentes: {list(df.columns)}")
    keep = [c for c in expected if c in df.columns]
    return df[keep].copy()

def copy_attachment_to_repo(src: str, class_code: str) -> str | None:
    if not src or str(src).strip() == "": return None
    p = Path(str(src))
...
def build_rows_html(df: pd.DataFrame, class_code: str) -> str:
    ...
    for _, row in df.iterrows():
        ...
        url = None
        try: url = copy_attachment_to_repo(str(pj), class_code)
...
def export_one_class(code: str, spec: dict) -> Path:
    ...
    if "Date" not in df.columns: raise KeyError("Date")
    df["Date"] = df["Date"].apply(coerce_date)

    today = pd.Timestamp(datetime.now().date())
    df = df[df["Date"].notna() & (df["Date"] <= today)]
    dbg(f"Après filtre date (<= {today.date()}): lignes={len(df)}")
    ...
    html = PAGE_TEMPLATE.format(
        title=title, today_fr=today_fr, now_fr=now_fr,
        rows_html=rows_html, table_style=TABLE_STYLE
    )
